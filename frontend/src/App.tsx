...
export default function App() {
  ...
  // Auto-Debits — read from accountsState (seeded & persisted in Firestore)
  const autoDebits = accountsState.autoDebits ?? [];
  ...
  const [setupCaregiverName, setSetupCaregiverName] = useState("Sara");
  const [setupCaregiverPhone, setSetupCaregiverPhone] = useState("+60 12-345 6789");
  const [setupElderlyLimit, setSetupElderlyLimit] = useState(300.00);
  const [setupRelationship, setSetupRelationship] = useState("Caregiver");
  const [setupAlertChannel, setSetupAlertChannel] = useState("NEURA App Direct Tap Notification");
  ...
  const fetchState = async () => {
    try {
      ...
      if (!res.ok) {
        console.error("Failed to sync state: HTTP", res.status);
        return;
      }
      ...
      if (typeof data?.totalBalance === "number") {
        ...
        console.error("Unexpected state shape from /api/state", data);
      }
    } catch (e) {
  ...
  const handleElderlyToggleClick = (enabled: boolean) => {
    if (enabled) {
      // Pre-fill setup modal from current state for easier editing
      setSetupCaregiverName(accountsState.caregiverName || "Sarafina");
      setSetupCaregiverPhone(accountsState.caregiverPhone || "+60 12-345 6789");
      setSetupElderlyLimit(accountsState.elderlyLimit || 300.00);
      setSetupStep("form");
      setTypedOtp("");
      setGeneratedOtp("");
                        ...
                        <div key={item.id} className="p-4 bg-slate-50/60 border border-slate-100/90 rounded-2xl flex flex-col justify-between hover:border-slate-200/50 transition-all select-none">
                          ...
                          <div className="flex items-center justify-between border-t border-slate-200/30 pt-3.5 mt-3.5">
                            ...
                            <div className="flex gap-1.5">
                              {item.status === "Active" ? (
                                <button
                                  onClick={() => handleToggleAutoDebit(item.id, item.name, item.status)}
                              ...
                              <button
                                onClick={() => handleDeleteAutoDebit(item.id, item.name)}
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-[#fff9ea] border border-[#f5dfb8] rounded-3xl p-5 space-y-3.5 text-[#854d0e] animate-fade-in duration-150">
              ...
              <p className="text-xs leading-relaxed font-bold">
                Semua butang besar dan mudah dibaca. Kami menyemak keselamatan setiap pemindahan wang untuk menghalang penipuan panggilan SMS atau WhatsApp. Penjaga anda sentiasa sedia menyemak pemindahan besar.
              </p>
              <div className="text-[10px] text-[#a16207]">
                Penjaga disambung: <strong>{accountsState.caregiverName}</strong> • {accountsState.caregiverPhone}
              </div>
            </div>
            ...
            <div className="overflow-y-auto flex-1 p-4 space-y-3 text-xs">
              ...
              {setupStep === "form" ? (
                ...
                <div className="space-y-3 font-sans">
                  ...
                  <div className="space-y-2.5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="space-y-1">
                        ...
                        <input 
                          ...
                          onChange={(e) => setSetupCaregiverPhone(e.target.value)}
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        <option value="NEURA App Push Alert">📱 NEURA Direct App Push Notification</option>
                        <option value="SMS Secure Code">💬 SMS Premium Secured OTP</option>
                        <option value="Verified WhatsApp Portal">🟢 WhatsApp Escrow Secure Verification Link</option>
                      </select>
                    </div>
                  </div>
                    ...
                    <button 
                      ...
                      onClick={() => {
                        if (!setupCaregiverName || !setupCaregiverPhone || setupElderlyLimit <= 0) {
                          alert("Sila masukkan maklumat lengkap.");
                          return;
                        }
                        // Generate and store random OTP
                        const randomCode = Math.floor(100000 + Math.random() * 900000).toString();
                        setGeneratedOtp(randomCode);
                        setTypedOtp("");
                        setOtpVerificationError("");
                ...
                <div className="space-y-3 font-sans">
                  ...
                  {/* Tied Caregiver End Simulated Screen Overlay */}
                  <div className="bg-slate-950 rounded-xl p-3 border border-slate-800 space-y-2 text-slate-100 relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between border-b border-slate-900 pb-1.5">
                      <span className="text-[7.5px] font-black tracking-widest text-[#d31145] font-mono uppercase">Caregiver Screen Preview</span>
                    ...
                    <div className="space-y-1 font-sans text-[10px]">
                      ...
                      <div className="bg-slate-900/80 p-2 rounded-lg border border-rose-955/40 text-center space-y-0.5 font-sans">
                        <span className="text-[8px] font-bold uppercase tracking-wider block text-slate-500 font-mono">Activation Code</span>
                        <div className="text-sm font-mono text-emerald-400 font-black tracking-wider bg-emerald-990 py-0.5 px-2.5 rounded border border-emerald-900/30 inline-block">
                          {generatedOtp}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2 shrink-0">
                    ...
                    <button 
                      ...
                      onClick={() => {
                        if (!typedOtp) {
                          setOtpVerificationError("Sila masukkan One-Time Code dari penjaga.");
                          return;
                        }
                        if (typedOtp !== generatedOtp) {
                          setOtpVerificationError("Kod Salah! Sila cuba lagi.");
                          return;
                        }
                ...
                <div className="space-y-3 font-sans">
                  ...
                  {/* Simulated Smartphone interface showing the Turn Off confirmation request */}
                  <div className="bg-slate-950 rounded-xl p-3 border border-slate-800 space-y-2 text-slate-100 relative overflow-hidden">
                    ...
                    <div className="space-y-1 font-sans text-[10px]">
                      ...
                      <div className="bg-slate-900/80 p-2 rounded-lg border border-amber-955/40 text-center space-y-0.5 font-sans">
                        <span className="text-[8px] font-bold uppercase tracking-wider block text-slate-500 font-mono">Deactivation Code</span>
                        <div className="text-sm font-mono text-amber-400 font-black tracking-wider bg-amber-950/45 py-0.5 px-2.5 rounded border border-emerald-900/30 inline-block">
                          {generatedOtp}
                        </div>
                      </div>
                    </div>
